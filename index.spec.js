...
describe('config', () => {
  ...
  it('fails promise when error', () => {
    AWS.mock('SSM', 'getParameters', (params) => {
      console.log('what th eheck')
      return {
        promise: () => {
          console.log('asdfasdfiu3387873')
        return Promise.reject(new Error('some AWS error'))
        }
      }
    })
    
    return config(['mysql.host']).then(blah => {
  ...
  describe('fetching values', () => {
    ...
    it('chunks the results if more than 10 parameters requested', () => {
